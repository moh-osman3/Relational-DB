...
./client < /cs165/generated_data/test${test_id}gen.dsl 2> ${output_dir}/test${test_id}gen.out.err 1> ${output_dir}/test${test_id}gen.out
cd /cs165
# run the "comparison" script for comparing against expected output for test_id
./infra_scripts/verify_output_standalone.sh $test_id ${output_dir}/test${test_id}gen.out /cs165/generated_data/test${test_id}gen.exp ${output_dir}/test${test_id}gen.cleaned.out ${output_dir}/test${test_id}gen.cleaned.sorted.out
